...
def main(BASIN,unit_conversion=1000,ET_scale = 1):
    '''
...
#     ET_scale = 0.1
    for i in range(len(df_P)):
        ...
        results['green_et_nu']=df_ETrain[df_ETrain.columns[0]].values[i]/unit_conversion*ET_scale
        results['green_et_au']=df_ETrain[df_ETrain.columns[1]].values[i]/unit_conversion*ET_scale
        results['green_et_uu']=df_ETrain[df_ETrain.columns[2]].values[i]/unit_conversion*ET_scale
        results['blue_et_nu']=df_ETincr[df_ETincr.columns[0]].values[i]/unit_conversion*ET_scale
        results['blue_et_au']=df_ETincr[df_ETincr.columns[1]].values[i]/unit_conversion*ET_scale
        results['blue_et_uu']=df_ETincr[df_ETincr.columns[2]].values[i]/unit_conversion*ET_scale
                
        #calculate water balance
        P=results['p_advection']
        ET=results['green_et_nu']+results['green_et_au']\
         +results['green_et_uu']+results['blue_et_nu']\
         +results['blue_et_au']+ results['blue_et_uu']
        
        results['landscape_et'] = results['green_et_nu']+results['green_et_au']\
                                 +results['green_et_uu']+results['blue_et_nu']\
                                 + results['blue_et_uu']
...
#         consumed_ditl = results['cw_do'] + results['cw_in'] + results['cw_to'] + results['cw_li']. #Consumed Water-domestic,industrial,tourism and livestock 
        consumed_ditl = results['cw_do']
        print('cw_do: ', consumed_ditl, (i+1))
        results['consumed_water_ditl'] = consumed_ditl
        ...
        if 'dS' not in BASIN['ts_data'].keys():
            ...
            results['dS']=calc_water_balance_residual(P,ET,Qin=Qin,Qout=Qout, Qdesal=results['q_in_desal'], consumed_ditl = consumed_ditl)
        ...
        # in case yearly dS and q_outflow is available calculate dS_error
            ...
            results['dS_error']=calc_water_balance_residual(P,ET,Qin=Qin,Qout=Qout,dS=results['dS'], consumed_ditl = consumed_ditl)
        ...
        #in case yearly dS is available, calculate q_outflow
            ...
            results['q_outflow']=calc_water_balance_residual(P,ET,
                   Qin=Qin,dS=results['dS'], consumed_ditl = consumed_ditl)
            
        # results['consumed'] = 0 #Consumed Water-domestic,industrial,tourism and livestock calculation
            
        results['manmade'] = results['blue_et_au'] + results['consumed_water_ditl']/unit_conversion
       # results['natural'] = results['blue_et_nu']
...
#         results['reserved_outflow_demand'] = q_out_avg * ewr_percentage       
        ...
        gross_inflow = results['p_advection'] + results['q_in_sw'] + results['q_in_gw'] + results['q_in_desal']
        net_inflow = gross_inflow + (-results['dS'])
...
#         results['mixed_out_sw'] = net_inflow - results['landscape_et'] - results['manmade']
        
#         results['net_inflow'] = net_inflow
...
def calc_water_balance_residual(P,ET,dS=None,Qin=None,Qout=None, Qdesal=None, consumed_ditl= None):
    if Qin is not None:
        gross_inflow=P+Qin
    else:
        gross_inflow=P
    ...
    if Qout is not None:
        dS=gross_inflow-ET-Qout-Qdesal-consumed_ditl
        return dS
...
def create_sheet1_csv(results, output_fh):
    ...
    writer.writerow(['STORAGE', 'CHANGE', 'Storage in sinks',
                     0.])
    writer.writerow(['OUTFLOW', 'ET RAIN', 'Natural',
                     '{0}'.format(results['green_et_nu'])])
    writer.writerow(['OUTFLOW', 'ET RAIN', 'Urban',
                     '{0}'.format(results['green_et_uu'])])
    writer.writerow(['OUTFLOW', 'ET RAIN', 'Agri',
                     '{0}'.format(results['green_et_au'])])   
    writer.writerow(['OUTFLOW', 'ET INCREMENTAL', 'Natural',
                     '{0}'.format(results['blue_et_nu'])])
    writer.writerow(['OUTFLOW', 'ET INCREMENTAL', 'Urban',
                     '{0}'.format(results['blue_et_uu'])])
    writer.writerow(['OUTFLOW', 'ET INCREMENTAL', 'Agri',
                     '{0}'.format(results['blue_et_au'])]) 
    writer.writerow(['OUTFLOW', 'ET INCREMENTAL', 'Manmade',
                     '{0}'.format(results['manmade'])])  
#     writer.writerow(['OUTFLOW', 'ET INCREMENTAL', 'Natural',
#                      '{0}'.format(results['natural'])]) 
    writer.writerow(['OUTFLOW', 'ET INCREMENTAL', 'Consumed Water',
                     '{0}'.format(results['consumed_water_ditl'])]) 
#     writer.writerow(['OUTFLOW', 'SURFACE WATER', 'Main riverstem',
#                      '{0}'.format(results['q_outflow'])])
    writer.writerow(['OUTFLOW', 'SURFACE WATER', 'Surface wateroutflow',
                     '{0}'.format(results['q_outflow'])])
    ...
    writer.writerow(['OUTFLOW', 'OTHER', 'Other',
                     0.])
    writer.writerow(['OUTFLOW', 'OTHER', 'Treated Waste Water', #Writing Treated Waste Water Value
                     '{0}'.format(results['tww'])]) 
