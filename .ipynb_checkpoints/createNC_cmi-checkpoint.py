...
def parsing_data(date, nc_file, overview, optionsProj, optionsClip, shape):
    var = dict()
...
#            sourceds = gdal.Warp(temp_file, fh[1], options = options)
    ...
    e = ThreadPoolExecutor(72)
    # threads = []
    var_lst = []
    dates_lst = []
    
    chunk_size = 100
    dates = sorted(overview.keys())
    chunked_list = [dates[i:i+chunk_size] for i in range(0, len(dates), chunk_size)]
    ...
    for chunk_dates in chunked_list:        
        for date in chunk_dates:
            var = e.submit(parsing_data, date, nc_file, overview, optionsProj, optionsClip, shape)        
            var_lst.append(var)
        ...
        fill_nc_all_timestep(nc_file, var_lst, shape)
    #     t = Thread(target=parsing_data, args=(date, nc_file, overview, optionsProj, optionsClip, shape))
    #     threads.append(t)
    #     t.start()
...
def fill_nc_all_timestep(nc_file, data, shape):
