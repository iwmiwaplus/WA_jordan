...
def create_sheet1(complete_data, metadata, output_dir, global_data):
    ...
    for date in common_dates:
        ...
        gwr.append(results['gwr'])
        ewr.append(results['ewr'])
        et.append(results['et'])
        p_recycle.append(results['p_recycled'])
        p_advec.append(results['p_advection'])
        q_in_sw.append(q_inflow)
        q_in_gw.append(0)
        q_in_desal.append(0)
        ds.append(results['dS'])
        q_outf.append(q_outflow)
        q_o_sw.append(q_transfer)
        q_out_avg_.append(q_out_avg)
        frac.append(results['non_utilizable_runoff'])
        rod.append(results['reserved_outflow_demand'])
        non_consumed.append(results['non_consumed_water'])
        
        non_recover.append(results['non_recoverable'])
        net.append(results['net_inflow'])
        consumed.append(results['consumed_water'])
        # Create the csv-file.
...
def calc_sheet1(entries, lu_fh, sheet1_lucs, recycling_ratio, q_outflow, q_out_avg, 
    ...
    results['et'] = ET
    results['gwr'] = gray_water_fraction
    results['ewr'] = ewr_percentage
    results['net_inflow'] =net_inflow
    results['consumed_water'] =consumed_water
    
    results['non_consumed_water'] =non_consumed_water
    results['non_utilizable_runoff'] = calc_non_utilizable(P, ET, entries['Fractions'])
