...
def WAPOR(output_folder, Startdate, Enddate, latlim, lonlim, Parameter, cores, Area = None, Version = "2"):
    ...
    args = [time_steps, output_folder_para, dimension, Parameter, latlim, lonlim, version, measure, Projection, url, header]
    # Loop over the dates
    if not cores:
        for Date_end in Dates_end:
            downloadData(Date_end, args)
    else:
        results = Parallel(n_jobs=cores)(delayed(downloadData)(Date_end, args)
                                         for Date_end in Dates_end)
        
    return ()

def downloadData(Date_end, args):
    print(Date_end)
    [time_steps, output_folder_para, dimension, Parameter, latlim, lonlim, version, measure, Projection, url, header] = args
        ...
        while success == 0 and no_succes<10:
            ...
            try:
                ...
                if job_response.status_code == 200:
                    while job_response.json()['response']['status'] == 'RUNNING' or job_response.json()['response']['status'] == 'WAITING':
                        time.sleep(20)
                        job_response = requests.get(job_url, headers=header)
