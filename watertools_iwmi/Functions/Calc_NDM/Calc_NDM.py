...
import watertools_iwmi.General.raster_conversions as RC
import watertools_iwmi.General.data_conversions as DC
...
def NPP_GPP_Based(Dir_Basin, Data_Path_GPP, Data_Path_NPP, Startdate, Enddate):
    ...
    for year in Years:
        ...
        yearly_NPP_File = glob.glob('*%d.01.01.tif' %int(year))[0]
...
#         Yearly_NPP[Yearly_NPP == NDV] = np.nan
        ...
        for Date in Dates:
            ...
            if Date.year == year:
                ...
                monthly_GPP_File = glob.glob(os.path.join('monthly','*monthly_%d.%02d.01.tif' %(int(year), int(month))))[0]
